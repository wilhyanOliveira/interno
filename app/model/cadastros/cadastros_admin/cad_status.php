...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(!empty($descricao))
    {
        ...
        $ver_existente = "SELECT COUNT(*) AS total FROM T_STATUS_ATEND WHERE descricao = '$descricao'";
        $consulta_setor = mysqli_query($conn, $ver_existente);
        $row = mysqli_fetch_assoc($consulta_setor);

        if($row['total'] > 0)
        {
            $mensagem = "Status ja cadastrado!";
        }
        else
        {
            $sql = "INSERT INTO T_STATUS_ATEND (descricao, status_setor) values ('$descricao', $status)";
        }
